 ...
 * generated code for so far. These can include the ones that have
 * not been transported to the target machine yet.
 */
map<string,int> global_data;

/*
 * transport_data keeps track of the code-variants that need to be
 * transported to the target machine for current search iteration.
 */
...
void generator_main(int i,vector<int> params, int prim) {
  //string path ("/fs/spoon/tiwari/code-server/scratch-space/");
...
void initialize() {
  ...
  get_file_list(file_list, script_directory+"file.list");
  ...
  sprintf(tmp_dir_path,"%s/tmp",working_directory.c_str());
  //if(debug_mode)
  ...
  for(int i=0; i<generator_vec.size(); i++){
    sprintf(tmp_dir_path,"%s/tmp/%s",working_directory.c_str(),(generator_vec.at(i)).c_str());
    if(debug_mode)
    ...
    for(int j=0; j<file_list.size(); j++) {
      sprintf(from_file, "%s/%s",script_directory.c_str(),
	      (file_list.at(j)).c_str());
      sprintf(to_file, "%s/tmp/%s/%s", working_directory.c_str(), 
  ...
  // transport_file.sh
  sprintf(from_file, "%s/transport_file.sh",script_directory.c_str());
  sprintf(to_file, "%s/transport_file.sh", working_directory.c_str());
  	printf("Copying %s to %s \n", from_file, to_file);	
  copy_file(from_file, to_file);
  // change the permisssions
  chmod(to_file, S_IRWXU);
  ...
  printf("Creating additional directories \n");
  // three additional directories: new_code, zipped, and transport
  sprintf(tmp_dir_path,"%s/tmp/new_code",working_directory.c_str());
  result=mkdir(tmp_dir_path,0755);
  ...
  sprintf(tmp_dir_path,"%s/tmp/zipped",working_directory.c_str());
  result=mkdir(tmp_dir_path,0755);
  ...
  sprintf(tmp_dir_path,"%s/tmp/transport",working_directory.c_str());
  result=mkdir(tmp_dir_path,0755);
...
void generator_manager_main(int server_pid_ ) {
   ...
   *    and usually holds true. For example, we usually use the idle lab machines.
   *    And all machines in our lab mount the same NFS.
   *
  ...
  string filename = script_directory+"generator_hosts";
  ...
  while(true) {
    ...
    */
    ...
    for (int i = 0; i < code_parameters.size(); i++)
      {
	/* 
	 * Check to make sure that the new batch of primary configurations have not
	 *  arrived yet. If they have, stop code-generation for the secondary confs.
	 */
	if(usr_interrupt) 
	  {
	    cout << "GOT A USER INTERRUPT!!! " << endl;
	    usr_interrupt=0;
	    break;
	  }
