...
hypre_SMGResidualCreate( )
{
   ...
   (residual_data -> time_index)  = hypre_InitializeTiming("SMGResidual_");
...
#include "hypre_box_smp_forloop.h"
          ...
          /* Only load the lib for the first time, reuse it later on */
                           ...
                           //FunctionLib =
                           //dlopen("/home/liao/svnrepos/benchmarks/smg2000/struct_ls/OUT__1__6755__.so",RTLD_LAZY);
                           FunctionLib = dlopen("/hivehomes/tiwari/rose_smg2000/offline/smg2000/test/OUT__1__6755__.so",RTLD_LAZY);
                           dlError = dlerror();
                          if( dlError ) {
