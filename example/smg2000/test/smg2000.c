...
#include "libcr.h"
  ...
  // client handle used across file
cr_client_id_t cr;
#endif
      ...
      char *argv[] ) 
{
   ...
   //double _time1__, _time2__;
   //_time1__=rt_clock();

   static double smg_residual_timing = 0.0;
#ifdef BLCR_CHECKPOINTING
  cr= cr_init();
  cri_info_init(); // ??? set cri_info_key
  printf("current process ID is %d\n",(int)getpid());
   ...
   if (solver_id == 0)
   {
      ...
      HYPRE_StructSMGSetMaxIter(solver, 22);  
      HYPRE_StructSMGSetTol(solver, 1.0e-14);
   ...
   //double time_elapsed_total;
   //time_elapsed_total=_time2__-_time1__;
   //printf(" %f \n", time_elapsed_total);
   MPI_Finalize();
   return (0);
}
