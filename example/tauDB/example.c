...
char * get_metadata() {
	...
	snprintf(retval, MAX_STR_LEN, "%s$$%s$$%s$$%s$$%d$$%s$$%s$$%s$$%s",uts.nodename, uts.sysname, uts.release, uts.machine, core_num, cpu_vendor, cpu_model, cpu_freq, cache_size);
...
int main(int argc, char **argv)
{
	...
	/* Inform the server about metadata */
	metadata = get_metadata();
	printf("Metadata is %s\n", metadata);
	harmony_inform(hdesc, "METADATA_CLIENT_0", metadata);
