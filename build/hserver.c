...
int server_startup()
{
  static const char* error_prefix = "<Error: server_startup @ hserver>";
  ...
  PID_temp = cfg_get("hspid_path"); // PHI_REMOVE_SYMBOL this is related to code-server
  ...
  if ( hs_pid )
    ...
    fh_exit(stderr, "%s: invalid hspid_path\n", error_prefix);  
  ...
  if ((listen_socket = socket (AF_INET, SOCK_STREAM,0)) < 0) 
    {
      ...
      fh_exit(stderr, "%s: failed to initialize socket\n", error_prefix);
    }
  ...
  if (setsockopt(listen_socket, SOL_SOCKET, SO_REUSEADDR, (char *) &optVal, sizeof(optVal))) 
    {
      ...
      fh_exit(stderr, "%s: failed to set socket options\n", error_prefix);
    }
  ...
  if (bind(listen_socket, (struct sockaddr *)&sin, sizeof(sin)) < 0) 
    {
      ...
      fh_exit(stderr, "%s: failed to bind socket.\n  Please check if there are previous \n instances of hserver still running.", error_prefix);
    }
  ...
  if ((err=Tcl_Init(tcl_inter)) != TCL_OK) 
    {
      fh_exit(stderr, "%s: Tcl Init failed!", error_prefix);
    }

  if ((err=Tcl_EvalFile(tcl_inter, harmonyTclFile)) != TCL_OK)
    {
      //printf("[AH]: Tcl Error %d ; %s %s \n",err, tcl_inter->result, harmonyTclFile);
      fh_exit(stderr, "%s: TCL Interpreter Error %d ; %s %s \n", error_prefix, err, tcl_inter->result, harmonyTclFile);
    }

  /*
    register update_client function with the Tcl interpretor
  */
  // PHI_REMOVE_SYMBOL TODO: do we still rely on update_client procedure?
  ...
  if (codeserver_type == NULL || strcmp(codeserver_type, "none") == 0) 
  ...
  else if (strcmp(codeserver_type, "standard") == 0) 
    {
      ...
      fh_exit(stderr, "%s: TCP-based code server not yet tested/implemented.", error_prefix);      
  ...
  else if (strcmp(codeserver_type, "standalone") == 0) 
    {
      ...
      if (codegen_init() < 0) 
	{
	  // PHI_REMOVE_SYMBOL why do we not crash here? 
	  return -1;
	}
    }
