...
void variable_request(HUpdateMessage *mesg, int client_socket)
{
    ...
    //HUpdateMessage *mesg = new HUpdateMesssage(HMESG_VAR_REQ, 0);
      ...
      for (int i=0; i<mesg->get_nr_vars();i++) 
	{
      ...
      //sprintf(s_updt, "updateObsGoodness %s %d %d", appName, *(int *)mesg->get_var(0)->getPointer(), mesg->get_timestamp());
	...
	//sprintf(ss_next, "get_next_configuration_by_name %s", appName);
	//sprintf(ss_next, "get_test_configuration %s", appName);
	 ...
	 switch(mesg->get_var(i)->getType())
	 {
      ...
      //  printf("New or next configuration: %s \n", next_config);
    ...
    if(it2 != it)
    {
      ...
      //printf("The server has not seen this config - (%s) before \n", next_config);
      //printf("Hence sending to the client(%d) for evaluation \n", clientId[client_socket]);

       //The search has concluded till the end and did not find the configuration
       //printf("The client(%d) gets the new config \n", clientId[client_socket]);
	 ...
	 return;
       ...
       //update the goodness:
       //sprintf(s_next_updt, "updateObsGoodness %s %d %d", appName, updt_perf, mesg->get_timestamp());
       
       // updating the goodness
       //sprintf(s_updt, "updateObsGoodness %s %d %d", appName, *(int *)mesg->get_var(0)->getPointer(), mesg->get_timestamp());


       //printf("YAHOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO \n");
       ...
       // before sending the message, start maintaining the state.
       // like a flag of some sort which tells the server that in the next iteration, 
       // you do not need to update the backend

       //performance_update_int(mesg, client_socket);

       //constructing a new message for sending back to the client
       // HUpdateMessage *mesg = new HUpdateMesssage(HMESG_PERF_ALREADY_EVALUATED, 0);
...
//  printf("Server gets the next configuration from the backend\n");
...
//  /*
//	Steps:
//	Iterate through all the Vardefs.. iterate through the VarDef objects in the message that you
//	just received.
//	For each VarDef, you need to call the get_next_configuration_by_name
//	Once you get the value, you need to set it to the VarDef (setting the value for 
//	different tunable parameters using the setValue.
   

//	Update the backend (using the updateObsgoodness)
//
//	Maintain a record saying that this particular appName got a conf other than the candidate
//	simplex.
//	Use this record in performance_update to decide whether or not to invoke updateObsgoodness.
//    */
...
//	 //   this will make sure that the multiple ply confs are unique.

	 //sprintf(ss_next, "get_next_configuration_by_name %s %s", appName, mesg->get_var(i)->getName());
//	 sprintf(ss_next, "get_next_configuration %s", appName);
...
//	     sprintf(&next_temp_name[next_endp-next_startp], "");
//	     sprintf(next_config, "%s_%s", next_temp_name, tcl_inter->result);
//	     //printf("Checking the map for next configuration: %s \n", next_config);
//	     printf("The next configuration that clients need to evaluate: %s \n", next_config);
//	   }
...
//	  //Maintaining the list of the new config associated to the appName
//	 //appName_confs.insert(pair<string, string>(appName, next_config));
//	 //global_data.insert(pair<string, string>(appName, next_config));
...
//       //printf("YAHOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO \n");
...
//	  //getting the perf from the map
//	  updated_performance = (*it2).second;
...
//	  if((err=Tcl_Eval(tcl_inter, s_next_updt)) != TCL_OK)
//	  {
//	     printf("FAILED HERE\n");
//	     operation_failed(client_socket);
//	     return;
...
//	 // before sending the message, start maintaining the state.
//	 //  like a flag of some sort which tells the server that in the next iteration, you do not need to update the backend
