...
//void performance_already_evaluated_double(HUpdateMessage *mesg, int client_socket)
//{

//  list<double>::iterator seen_performance;

//  double performance = FLTMAXVAL;

//  char* perf_dbl=(char*)(mesg->get_var(0)->getPointer());

//  performance=atof(perf_dbl);

//  for (seen_performance = global_data.begin(); seen_performance != global_data.end(); seen_performance++)
//   {
//      if(*(seen_performance) == performance))
//      {

//	printf("The particular performance has already seen before and hence no further performance evaluation is done \n");

//       return seen_performance;
