...
void variable_request(HUpdateMessage *mesg, int client_socket)
{
	...
	// once you get the brand new conf from the backend, check if the conf exists in the
	//  global database.
	//  If it does. loop.
	// if not, break.
	...
	printf("  Parameter value: %s\n ", result);
        ...
        //if(result[0]==0 || result[0]==1 || result[0]==2 || result[0]==3 || result[0]==4 || result[0]==5 || result[0]==6 || result[0]==7 || result[0]==8 || result[0]==9 )
	//{
	     //printf("  Parameter value: %s\n ", result);
	     if(result != NULL)
	     while(result != NULL)
	     ...
	     //}
	     //else if(result[0]=='a' || result[0]=='b' || result[0]=='c' || result[0]=='d' || result[0]=='e' || result[0]=='f' || result[0]=='g' || result[0]=='h' || result[0]=='i' || result[0]=='j' || result[0]=='k' || result[0]=='l' || result[0]=='m' || result[0]=='n' || result[0]=='o' || result[0]=='p' || result[0]=='q' || result[0]=='r' || result[0]=='s' || result[0]=='t' || result[0]=='u' || result[0]=='v' || result[0]=='w' || result[0]=='x' || result[0]=='y' || result[0]=='z')
	     //{
	     //if(result != "")
	     //{
	       //NEW_STR = result;
	       ...
	       //This part of the code is used for debugging.
	       //To check if the parameter value is a string or letters instead of numbers.
	       ...
	       //result = strtok("", delims);
	     //}
	     //}
	   
	   //printf("YAHOOOOOOOOOOOUoooooooooooUUUUuuuuuuuuuuuuuuUUUUUHHHHHHHHHHHhhhhhhhhhhhhhhhhHHHHHHHHHHHHHHHhEEEEEeeeeeeeeeeeeeEEEEEEEEE \n");
	

	   //it2 = appName_confs.find(ss_next);

	   //if(((*it2).second) != ((*it).first))
	   //{

	    //printf("Server issues ****UNIQUE CONFIG**** to the clients for evaluating\n");
	    //printf("Server confirms it as the *****UNIQUE CONFIG***** \n");
		...
		{
	          case VAR_INT:
		    //printf("SETTING THE VALUE TO THE MESSAGE YAHOOOOOOOOOOOUUUUUUUUUUHEEEEEEEEEEEEEE \n");
		    //printf("Setting the new value to the tunable parameter \n");
    ...
    //       int new_length = strlen(result);
	     
    //       for (int k=0; k<mesg->get_nr_vars();k++)
    //      {

    //	//NEW_STR_CPY[k] = NEW_STR[k];
		
    //	for(int ru=0; ru<new_length; ru++)
    //	{
    //	    NEW_STR_CPY[ru] = NEW_STR[ru];
    //	}
	
    //	int brand_new_string = atoi(NEW_STR_CPY);
    ...
    //	    break;
    //          case VAR_STR:
    //	    mesg->get_var(k)->setValue(brand_new_string);
    //	    break;
    //	}
    ...
    //    } 
	 ...
	 //{
	   ...
	   //printf("Server is unable to issue ****UNIQUE CONFIG**** to the clients \n");
	   ...
	   //sprintf(ns, "updateObsGoodness %s %d %d", appName,*(int *)mesg->get_var(0)->getPointer(), mesg->get_timestamp());
