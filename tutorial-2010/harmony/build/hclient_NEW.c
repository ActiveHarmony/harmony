...
// Old design - Client waits for the new "configuration" from the "server" after sending the performance values
//  When the "server" sends the "configuration" the "client" queries the "server" whether it has seen this before.
// "Server" now queries the "database" to check whether it has seen this "configuration" before. If the "database" has seen this
// "configuration" before it will respond back and further the "server" responds back to the "client" with the final "configuration"

// New design - As per the new design the "server" gets the "configuration" request from the "client" and queries the "database"
// directly whether it has seen this particular "performance" values and corresponding "configuration" before. If the "database" has seen this "configuration"
// it will respond back, further the "server" responds back to the "client" with the final "configuration".

//void harmony_request_all(int socketIndex, int pull)
