 ...
 *
 * This processing layer takes a log of point/performance pairs as
 * generated by log.so. If points requested by the strategy are
 * present in the log file, this layer returns the point/performance
 * pair in the log to the strategy instead of sending the point out to
 * get tested by the client. Otherwise, the requested point is sent
 * out for normal testing by the client(s).  Note that the client and
 * the Harmony server are not informed about points found in the
 * cache.
 *
...
int cache_analyze(hflow_t *flow, htrial_t *trial)
{
    if (!skip) {
        ...
        cache[cache_len].perf = trial->perf;
        ++cache_len;
    }
    skip = 0;
...
int load_logger_file(const char *filename)
{
    ...
    while (fscanf(fp, " %c", &c) != EOF) {
        ...
        if (fscanf(fp, " ) => (%*f[%la])", &cache[cache_len].perf) != 1) {
